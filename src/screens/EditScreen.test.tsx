...
describe('App', () => {
  ...
  it('redirects a bare /edit to the remembered last-viewed story, when it still exists', async () => {
    ...
    expect(await screen.findByDisplayValue('The Wheel of Time')).toBeInTheDocument();
  });

  it('falls back to the create-new flow for a URL naming a non-numeric story id', async () => {
    render(
      <MemoryRouter initialEntries={['/edit/not-a-real-id']}>
        <App />
      </MemoryRouter>,
    );
